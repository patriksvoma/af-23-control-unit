...
void RideStats::update()
{
    ...
    // Keep max RPM
    if (motor::getRPM() > max_rpm)
        max_rpm = motor::getRPM();
    ...
    if (timing_active)
    {
        ...
        if (fabs(speed_mps - last_recorded_speed) < 0.5f)
        {
            ...
            else if (current_time - stable_speed_start > 3000000)
            { // 3 seconds of stable speed
                timing_active = false;
        ...
        if (current_time - accel_start_time > 60000000)
        { // 60 seconds max
            timing_active = false;
